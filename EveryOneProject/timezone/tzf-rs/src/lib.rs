/*
--TODO:
        属性指令，告诉Rust编译器要将项目的文档注释设置为README.md文件的内容。include_str!是一个宏，他会在编译时将指定文件的内容嵌入到
    生成的二进制文件中。这个属性指令的目的是将README.md文件中的内容作为文档注释，用于生成项目的文档。
*/
...
impl Finder {
    ...
    // -> 整个函数标签中#[must_use]是一个属性(attribute),用于提示调用者应该使用方法的返回值
    #[must_use]
